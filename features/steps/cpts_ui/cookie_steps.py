...
def i_am_on_the_cookies_page(context):
    context.active_page.goto(f"{context.cpts_ui_base_url}site/cookies")


@when("I press the {button_name} button")
def i_press_a_cookie_button(context, button_name):
    button_test_id = f"{button_name}-button"
    button = context.active_page.get_by_test_id(button_test_id)
...
def i_click_the_policy_link(context):
    link = context.active_page.get_by_test_id("cookieInfoLink")
    link.click()
...
def i_click_the_small_banner_policy_link(context):
    page_link = context.active_page.get_by_test_id("smallCookieBannerLink")
    page_link.click()
...
def i_expand_the_cookies_info_table(context, table_type):
    ...
    table_dropdown = context.active_page.get_by_test_id(cookies_dropdown_text)
...
def i_choose_cookie_option(context, option):
    if option == "use":
        radio_button = context.active_page.get_by_test_id("accept-analytics-cookies")
    else:
        radio_button = context.active_page.get_by_test_id("reject-analytics-cookies")
    expect(radio_button).to_be_visible()
    radio_button.click()

    save_button = context.active_page.get_by_test_id("save-cookie-preferences")
...
def i_see_the_relevant_table(context, table_type):
    ...
    table = context.active_page.get_by_test_id(table_test_id)
...
def i_do_not_have_rum_cookies(context):
    cookies = context.active_page.context.cookies()
