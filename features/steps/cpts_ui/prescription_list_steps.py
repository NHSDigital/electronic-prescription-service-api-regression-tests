...
def search_context_prescription_id(context):
    ...
    context.active_page.get_by_test_id("find-prescription-button").click()
...
def search_context_nhs_number(context):
    ...
    search_input = context.active_page.get_by_test_id("nhs-number-input")
    search_input.fill(nhs_number)
    context.active_page.get_by_test_id("find-patient-button").click()
...
def i_see_the_showing_count_text_if_relevant(context):
    summary_row = context.active_page.get_by_test_id("table-summary-row")
...
def sort_table_by_column(context, column_name):
    ...
    header_locator = context.active_page.locator(f'[data-testid="{column_key}"]')
    ...
    if initial_direction == "descending" and target_direction == "ascending":
        ...
        header_locator.click()
        context.active_page.wait_for_timeout(500)
        header_locator.click()
...
def check_url_redirect_for_prescriptions(context):
    ...
    context.active_page.wait_for_url(expected_url_pattern)
