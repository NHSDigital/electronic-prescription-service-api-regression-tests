...
def i_enter_text_in_input_box(context):
    search_box = context.active_page.get_by_test_id("prescription-id-input")
    search_box.click()
...
def redirected_to_nhs_current(context, nhs_number):
    ...
    context.active_page.wait_for_url(expected_url, wait_until="load", timeout=60000)
...
def click_first_error_link(context):
    context.active_page.locator('[data-testid="error-summary"] a').first.click()


@then('the focus should be on the "{field_id}" input')
def assert_focus_on_input(context, field_id):
    active = context.active_page.evaluate("document.activeElement.id")
