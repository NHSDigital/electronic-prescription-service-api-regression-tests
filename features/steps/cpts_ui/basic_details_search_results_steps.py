...
def navigate_to_search_results(context):
    ...
    context.active_page.page.goto(
        context.cpts_ui_base_url + context.active_page.url.lstrip("/")
    )
...
def verify_search_page_navigation(context):
    expect(context.active_page.page).to_have_url(
...
def verify_main_content_role(context, role):
    assert (
        context.active_page.get_main_content_role() == role
...
def verify_table_cell_headers(context):
    # Check name cell
    name_cell_headers = context.active_page.get_table_cell_headers(
        "Issac Wolderton-Rodriguez"
    )
    assert (
