...
from eps_test_support.pages.basic_details_search_results import (
...
def navigate_to_search_results(context):
    context.active_page = BasicDetailsSearchResultsPage(context.active_page)
    context.active_page.page.goto(context.cpts_ui_base_url + context.active_page.url.lstrip("/"))
    context.active_page.wait_for_page_load()
...
def verify_table_columns(context):
    ...
    actual_columns = context.active_page.get_table_headers()
    assert all(
...
def verify_table_sorting(context):
    rows = context.active_page.get_patient_rows()
...
def verify_results_count(context, expected_text):
    actual_text = context.active_page.get_results_count_text()
    # Handle both formats of the results count text
    assert (
        expected_text in actual_text or f"We found {expected_text.split()[0]} results" in actual_text
...
def verify_no_restricted_patients(context):
    assert not context.active_page.is_restricted_patient_visible(
        "John Smith"
...
def click_patient_row(context, name):
    context.active_page.click_patient_row(name)


@when('I press enter on the patient row for "{name}"')
def press_enter_patient_row(context, name):
    context.active_page.press_enter_on_patient_row(name)
...
def verify_prescription_list_navigation(context):
    ...
    context.active_page.page.wait_for_url(
        lambda url: "prescription-list-current" in url or "prescription-not-found" in url
    )
    current_url = context.active_page.page.url
    assert (
        "prescription-list-current" in current_url or "prescription-not-found" in current_url
...
def verify_nhs_number_in_url(context, nhs_number):
    ...
    # Wait for navigation and check for either prescription list or not found page
    context.active_page.page.wait_for_url(lambda url: formatted_nhs in url)
    current_url = context.active_page.page.url
    assert formatted_nhs in current_url, f"Expected NHS number {formatted_nhs} in URL, got: {current_url}"


@when('I click the "{link_text}" link')
def click_back_link(context, link_text):
    context.active_page.click_go_back()


@then("I should be navigated to the basic details search page")
def verify_search_page_navigation(context):
    expect(context.active_page.page).to_have_url(context.cpts_ui_base_url + "site/search-by-basic-details")
...
def verify_search_form_cleared(context):
    ...
    expect(context.active_page.page).to_have_url(f"{context.cpts_ui_base_url}site/search-by-basic-details")


@then("the table should be responsive")
def verify_table_responsive(context):
    assert context.active_page.is_table_responsive(), "Table is not responsive"


@then('the main content should have the role "{role}"')
def verify_main_content_role(context, role):
    assert context.active_page.get_main_content_role() == role, f"Main content role is not {role}"


@then('the results header should have the id "{id}"')
def verify_results_header_id(context, id):
    expect(context.active_page.results_header).to_have_id(id)


@then('the results count text should have the id "{id}"')
def verify_results_count_id(context, id):
    expect(context.active_page.results_count).to_have_id(id)


@then("table cells should have the correct headers attribute")
def verify_table_cell_headers(context):
    # Check name cell
    name_cell_headers = context.active_page.get_table_cell_headers("Issac Wolderton-Rodriguez")
    assert name_cell_headers == "header-name", f"Expected header-name, got {name_cell_headers}"

    # Check gender cell
    gender_cell_headers = context.active_page.get_table_cell_headers("Male")
    assert gender_cell_headers == "header-gender", f"Expected header-gender, got {gender_cell_headers}"

    # Check date of birth cell
    dob_cell_headers = context.active_page.get_table_cell_headers("6-May-2013")
    assert dob_cell_headers == "header-dob", f"Expected header-dob, got {dob_cell_headers}"

    # Check address cell
    address_cell_headers = context.active_page.get_table_cell_headers(
        "123 Brundel Close, Headingley, Leeds, West Yorkshire"
    )
    assert address_cell_headers == "header-address", f"Expected header-address, got {address_cell_headers}"

    # Check NHS number cell - look for the cell containing the NHS number
    nhs_cell_headers = context.active_page.get_table_cell_headers("972 691 9207")
    assert nhs_cell_headers == "header-nhs", f"Expected header-nhs, got {nhs_cell_headers}"


@then('the NHS number "{nhs_number}" should have visually hidden text')
def verify_visually_hidden_nhs_number(context, nhs_number):
    formatted_nhs = nhs_number.replace(" ", "")
    hidden_text = context.active_page.get_visually_hidden_text(formatted_nhs)
    assert hidden_text == f"NHS number {nhs_number}", f"Expected 'NHS number {nhs_number}', got {hidden_text}"
