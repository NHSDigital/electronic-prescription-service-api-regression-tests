...
def login_with_multiple_access_roles(context):
    assert (
        "@multiple_access" not in context.config._tags
    ), "Trying to use MOCK_CIS2_LOGIN_ID_MULTIPLE_ACCESS_ROLES in a test not tagged @multiple_access"
...
def login_with_no_roles(context):
    assert (
        "@no_roles_no_access" not in context.config._tags
    ), "Trying to use MOCK_CIS2_LOGIN_ID_NO_ROLES in a test not tagged @no_roles_no_access"
...
def login_with_without_access(context):
    assert (
        "@multiple_roles_no_access" not in context.config._tags
    ), "Trying to use MOCK_CIS2_LOGIN_ID_NO_ACCESS_ROLE in a test not tagged @multiple_roles_no_access"
...
def login_with_single_role_with_access_multiple_without(context):
    assert (
        "@multiple_roles_single_access" in context.config._tags
...
def login_with_pre_role_selected(context):
    assert (
        "@multiple_access_pre_selected" in context.config._tags
...
def login_with_single_role(context):
    assert (
        "@single_access" in context.config._tags
    ), "Trying to use MOCK_CIS2_LOGIN_ID_SINGLE_ACCESS_ROLE in a test not tagged @single_access"
